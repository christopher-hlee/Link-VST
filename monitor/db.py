...
Follows the LinkVST convention (raw sqlite3, WAL, row_factory=Row) with one
deliberate difference: connections are closed explicitly. The scheduler opens a
connection every tick, and `with sqlite3.connect(...)` commits but does not
close, so the plain pattern would leak file descriptors in a long-running process.
"""
import json
import sqlite3
