...
The dashboard is reachable from the public internet, so the Bearer-key pattern
LinkVST uses for machine clients is not enough on its own: a browser cannot hold
a secret. Sessions are signed cookies; the password is stored only as an scrypt
hash, generated on the server via `python -m monitor.hashpw`.
"""
import hashlib
import hmac
import os
